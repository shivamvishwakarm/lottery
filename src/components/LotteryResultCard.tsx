...
import AnimatedBallons from "./AnimatedBallons";
...
const LotteryResultCard: React.FC<LotteryResultCardProps> = ({
  name,
  winPrice,
  ticket,
  date,
  ...
  let formattedDate = `${drawDate.getDate()}/${
    drawDate.getMonth() + 1
  ...
  return (
    <div
      ...
      <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl mt-8 font-bold text-blue-600">
        Kerala Lottery Results
      </h1>

      <div className="flex flex-col justify-center items-center bg-blue-700 my-6 rounded w-full md:w-1/2 h-auto text-white">
        <h2 className=" text-2xl font-bold pt-8 ">Congratulations</h2>
        <AnimatedBallons />
        <h2
          className={`my-2 text-yellow-400 text-4xl font-bold ${styles.blink}`}>
          {capitalName}
        </h2>
        {/* Apply the blink class here */}
        <p className="my-2 font-medium px-2">
          You Have won the 4th Prize for{" "}
          <span className="underline">Rs. {winPrice}/-</span>
        </p>
        <p className="my-2 font-semibold">Ticket no: {ticket}</p>
        <p className="my-2 text-sky-400 text-semibold">
